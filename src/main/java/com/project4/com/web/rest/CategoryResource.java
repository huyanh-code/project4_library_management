...
public class CategoryResource {
     ...
     */
    @GetMapping
    public ResponseEntity<Page<CategoryDTO>> getAll(@org.springdoc.core.annotations.ParameterObject Pageable pageable) {
        LOG.debug("REST request to get all readers for an admin");
        if (!onlyContainsAllowedProperties(pageable)) {
            return ResponseEntity.badRequest().build();
        }

        final Page<CategoryDTO> page = categoryService.getAllCategory(pageable);
