...
public class AuthorResource {
     ...
     */
    @GetMapping
    public ResponseEntity<Page<AuthorDTO>> getAll(@org.springdoc.core.annotations.ParameterObject Pageable pageable) {
        LOG.debug("REST request to get all readers for an admin");
        if (!onlyContainsAllowedProperties(pageable)) {
            return ResponseEntity.badRequest().build();
        }

        final Page<AuthorDTO> page = authorService.getAllAuthors(pageable);
